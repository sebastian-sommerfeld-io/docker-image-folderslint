...
== What is `{image-name}`?
The `{image-name}` image is a utility Docker image used in Github Actions workflows to validate the directory structure of a project. The image ships with link:https://www.npmjs.com/package/folderslint[folderslint].
